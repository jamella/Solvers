...
import os
import time
# from testforbid_single import CBconstrain
...
def genPO4Orac(OracBlif, PIvec, assignOracBlif, subPart=None):
...
def oracV2cnf(OracInV, header=True):
    ...
    if header==True:
        ...
        clauseNum += len(inputs) # add the number of PI assignment clauses in advance
    ...
    return (inputs[0], posIndex, cnFile, varNum)
...
def v2cnfMtr(camInV, MuxStyle, MuxStyle_flag, header=True):
    ...
    for line in Vlines:
        ...
        if 'input' in line:
            ...
            wires=re.search(r'(?<=wire )(.*)(?=$)', line).group().replace(' ','').split(',')
            for w in wires:
                varIndexDict[w.replace('\t','')] = varIndex
                #intVarDict[varIndex] = w
                varIndex += 1
        elif line!='' and line[0]!='/' and not 'module' in line:
            #print varIndexDict
    ...
    #!! Don't use the following line, because in Minisat assumption is used !!
    ...
    return (cnFile, inputs, posIndex, camVarNum, camCNFile, varNum)
